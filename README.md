...
2. 不改加速器：程序网络选 **「浏览器」**（或保持「自动」，它会自动兜底到浏览器）。程序会在后台用本机 Chrome / Edge 的无头模式抓数据，借用加速器给浏览器开的通道。加速器一般只放行**默认浏览器**，程序会优先用它，也可以在「浏览器」下拉框里手动指定；每次请求要拉起一次浏览器，比直连慢一些，且不加载封面图。

**不知道哪里出问题？** 点主程序右上角的「网络诊断」（或运行 `python debug_network.py`），它会逐项检查代理设置、DNS、TCP/TLS、HTTP、本机代理端口、curl 与无头浏览器对比、加速器痕迹，并给出结论；点「复制报告」把全文发给 Claude 即可定位。
